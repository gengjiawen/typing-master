...
function App() {
  ...
  const applyDecorations = useCallback(
    (code: string, input: string) => {
      ...
      if (editorRef.current && monacoRef.current && targetPosition) {
        // Ensure refs and targetPosition are valid
    ...
    [setAccuracy],
  ) // Add setAccuracy as it's used from useState
  ...
  const handleKeyDown = useCallback(
    ...
    [
      userInput,
      setUserInput, // Jotai atom value and setter
      isTyping,
  ...
  const handleEditorDidMount: OnMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('typing-theme-dark', {
      base: 'vs-dark',
      inherit: true,
      rules: [],
      colors: { 'editor.background': '#1e1e1e', 'editor.foreground': '#d4d4d4' },
    })
    monaco.editor.defineTheme('typing-theme-light', {
      base: 'vs',
      inherit: true,
      rules: [],
  ...
  return (
    ...
    <div
      ref={appContainerRef}
      tabIndex={-1}
      className='min-h-screen p-4 sm:p-8 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:outline-none'
    >
      {/* Center content within the full-width container */}
      <div className='max-w-5xl mx-auto'>
        {' '}
        {/* Optional: Reintroduce a max-width for content if needed, or remove */}
